"""Single-file PyTorch implementation of Cal-QL.

This file ports the author-released JAX/Flax implementation of:

    Cal-QL: Calibrated Offline RL Pre-Training for Efficient
    Online Fine-Tuning (NeurIPS 2023).

The implementation keeps the user's existing single-file PyTorch style while
following both the paper and the official repository for the released domains.
The main algorithm is SAC+CQL with one calibrated change to the conservative
regularizer: policy-sampled Q values are lower-bounded by a reference-policy
value before the CQL log-sum-exp is computed.

For the sparse episodic tasks released by the authors, the reference-policy
value is the Monte-Carlo return-to-go of the behavior trajectory. The file
implements the official AntMaze path directly and includes an optional loader
for the author's Adroit-Binary ``.npy`` demonstrations. The author repository
does not release its locomotion implementation; using trajectory Monte-Carlo
returns on dense D4RL locomotion is therefore disabled by default and must be
explicitly enabled as a non-official extension.

Key implementation details matched to the official source:

1. Tanh-Gaussian SAC actor with learnable log-standard-deviation multiplier
   and offset, twin Q-functions, automatic entropy tuning, and Polyak targets.
2. Max-Q target backup over multiple next-policy actions (10 by default), with
   entropy backup disabled in the released AntMaze and Adroit configurations.
3. CQL importance-sampled random/current/next policy actions.
4. Cal-QL lower-bounds only policy-sampled conservative Q values by the stored
   Monte-Carlo return; random-action and data-action Q values are not clamped.
5. Optional Lagrange CQL coefficient with the official AntMaze target action
   gap of 0.8.
6. Offline pre-training followed by online fine-tuning with a fixed mixture of
   separate offline and online replay buffers (0.5/0.5 in released settings).
7. Online transitions are collected by complete trajectory so their return-to-
   go can be computed before insertion, matching the official sparse-task code.
8. Default AntMaze preset: reward_scale=10, reward_bias=-5, CQL weight=5,
   Lagrange enabled, 1,000,000 offline updates, and 1,000,000 online steps.

PyTorch and JAX/Flax use different random-number streams, parameter layouts,
and low-level kernels. Consequently, identical seeds do not produce bitwise-
identical trajectories or parameters. This is an algorithmically faithful
PyTorch port of the released Cal-QL implementation, not a numerical bit-for-bit
reproduction.
"""
...
@dataclass
class TrainConfig:
    # Official repository default in this port: released AntMaze configuration.
    env_name: str = "antmaze-medium-diverse-v2"
    official_preset: bool = True

    # Dataset source: d4rl | adroit_binary.
    dataset_source: str = "d4rl"
    adroit_binary_data_dir: Optional[str] = None
    adroit_binary_include_bc: bool = True

    # The official repository only releases AntMaze and Adroit-Binary paths.
    # Generic dense D4RL trajectory returns are not the locomotion reference
    # estimator described by the paper and are disabled unless opted into.
    allow_generic_mc_reference: bool = False

    # Network widths are resolved by the official preset when None:
    # AntMaze actor 256x2, Q 256x4; Adroit actor 512x2, Q 512x3.
    ...
    # Official code optimizes log-alpha-prime with the Q-function LR.
    ...
    # CQL / Cal-QL settings from the released ConservativeSAC implementation.
    ...
    # SAC target backup. Released sparse-task code uses max target backup and
    # disables entropy backup.
    max_target_backup: bool = True
    backup_entropy: bool = False
    target_backup_n_actions: int = 10

    # Reward transform is applied before both TD targets and return-to-go.
    reward_scale: Optional[float] = None
    reward_bias: Optional[float] = None
    sparse_failure_reward: Optional[float] = None

    # Offline-to-online schedule.
    pretrain_steps: Optional[int] = None
    num_total_steps: Optional[int] = None
    online_utd_ratio: int = 1
    # Official flag semantics: fraction of each online-phase batch drawn from
    # the fixed offline dataset; the remainder comes from online replay.
    mixing_ratio: float = 0.5
    online_use_cql: bool = True
    cql_min_q_weight_online: Optional[float] = None
    replay_buffer_size: int = 1_000_000

    # Official online loop collects complete trajectories, then performs
    # trajectory_length * UTD updates.
    trajectories_per_iteration: int = 1

    eval_episodes: int = 20
    offline_eval_every: Optional[int] = None
    online_eval_every: Optional[int] = None
    log_every: int = 1_000
    ...
    log_root: str = "logs/CalQL_PyTorch"
...
def _is_adroit_binary(env_name: str, source: str) -> bool:
    return source.lower() == "adroit_binary" or "binary" in env_name.lower()
...
def _resolve_official_preset(config: TrainConfig) -> None:
    ...
    if source not in {"d4rl", "adroit_binary"}:
        raise ValueError("dataset_source must be 'd4rl' or 'adroit_binary'.")
    config.dataset_source = source

    if not config.official_preset:
        return

    if _is_antmaze(config.env_name):
        if config.actor_hidden_dims is None:
            config.actor_hidden_dims = (256, 256)
        if config.critic_hidden_dims is None:
            config.critic_hidden_dims = (256, 256, 256, 256)
        if config.cql_min_q_weight is None:
            config.cql_min_q_weight = 5.0
        if config.cql_lagrange is None:
            config.cql_lagrange = True
        if config.cql_target_action_gap is None:
            config.cql_target_action_gap = 0.8
        if config.reward_scale is None:
            config.reward_scale = 10.0
        if config.reward_bias is None:
            config.reward_bias = -5.0
        if config.sparse_failure_reward is None:
            # Original D4RL AntMaze failure reward is 0; after transform it is -5.
        ...
        if config.num_total_steps is None:
            config.num_total_steps = 1_000_000
        if config.offline_eval_every is None:
            config.offline_eval_every = 50_000
        if config.online_eval_every is None:
            config.online_eval_every = 2_000

    elif _is_adroit_binary(config.env_name, source):
        if config.actor_hidden_dims is None:
            config.actor_hidden_dims = (512, 512)
        if config.critic_hidden_dims is None:
            config.critic_hidden_dims = (512, 512, 512)
        if config.cql_min_q_weight is None:
            config.cql_min_q_weight = 1.0
        if config.cql_lagrange is None:
            config.cql_lagrange = False
        if config.cql_target_action_gap is None:
            config.cql_target_action_gap = 0.0
        if config.reward_scale is None:
            config.reward_scale = 10.0
        if config.reward_bias is None:
            config.reward_bias = 5.0
        if config.sparse_failure_reward is None:
            # Binary Adroit uses -1 until success; transformed failure is -5.
            config.sparse_failure_reward = -5.0
        if config.pretrain_steps is None:
            config.pretrain_steps = 20_000
        if config.num_total_steps is None:
            # The released run script uses 200k for Pen and 1m otherwise.
            config.num_total_steps = (
                200_000 if "pen" in config.env_name.lower() else 1_000_000
            )
        if config.offline_eval_every is None:
            config.offline_eval_every = 2_000
        if config.online_eval_every is None:
            config.online_eval_every = 2_000

    else:
        # The author repository does not release this path. Keep generic values,
        # but require explicit opt-in for trajectory MC calibration.
        if config.actor_hidden_dims is None:
            config.actor_hidden_dims = (256, 256)
        if config.critic_hidden_dims is None:
        ...
        if config.offline_eval_every is None:
            config.offline_eval_every = 50_000
        if config.online_eval_every is None:
            config.online_eval_every = 5_000


def _validate_config(config: TrainConfig) -> None:
    _resolve_official_preset(config)

    if config.actor_hidden_dims is None or config.critic_hidden_dims is None:
        raise RuntimeError("Network dimensions were not resolved.")
    if config.cql_min_q_weight is None or config.cql_lagrange is None:
        raise RuntimeError("CQL configuration was not resolved.")
    if config.cql_target_action_gap is None:
        raise RuntimeError("CQL target gap was not resolved.")
    if config.reward_scale is None or config.reward_bias is None:
        raise RuntimeError("Reward transform was not resolved.")
    if config.pretrain_steps is None or config.num_total_steps is None:
        raise RuntimeError("Training schedule was not resolved.")
    if config.offline_eval_every is None or config.online_eval_every is None:
        raise RuntimeError("Evaluation schedule was not resolved.")

    positive_ints = {
        "batch_size": config.batch_size,
        "cql_n_actions": config.cql_n_actions,
        "target_backup_n_actions": config.target_backup_n_actions,
        "pretrain_steps": config.pretrain_steps,
        "num_total_steps": config.num_total_steps,
        "online_utd_ratio": config.online_utd_ratio,
        "trajectories_per_iteration": config.trajectories_per_iteration,
    ...
    if _is_locomotion(config.env_name) and not config.allow_generic_mc_reference:
        raise ValueError(
            "The official Cal-QL repository does not release the locomotion "
            "reference-value estimator. The paper uses a fitted SARSA Q lower "
            "bound rather than trajectory MC returns. Set "
            "allow_generic_mc_reference=True only to run the explicitly "
            "non-official trajectory-return extension."
        )
...
def _load_d4rl_trajectory_dataset(env: gym.Env, config: TrainConfig) -> NumpyBatch:
    candidate = env
    raw_dataset = None
    for _ in range(16):
    ...
    for index in range(total):
        ...
        # Exact released-loader semantics: skip timeout transitions and the
        # final raw dataset element, then close the accumulated episode below.
        if not final_timestep and index != total - 1:
            ...
            if "next_observations" in raw_dataset:
                ep_next_obs.append(raw_dataset["next_observations"][index])
            else:
...
def _binary_file_candidates(env_name: str, include_bc: bool) -> List[str]:
    name = env_name.lower()
    if "pen" in name:
        expert = "pen2_sparse.npy"
        bc = "pen_bc_sparse4.npy"
    elif "door" in name:
        expert = "door2_sparse.npy"
        bc = "door_bc_sparse4.npy"
    elif "relocate" in name:
        expert = "relocate2_sparse.npy"
        bc = "relocate_bc_sparse4.npy"
    else:
        raise ValueError(
            "Adroit-Binary loader supports Pen, Door, and Relocate tasks."
        )
...
def _extract_trajectory_field(trajectory, names: Sequence[str]):
    ...
    raise KeyError(f"None of the trajectory fields {tuple(names)} were found.")
...
def _state_observation_array(values) -> np.ndarray:
    ...
    for value in values:
        if isinstance(value, Mapping):
            if "state_observation" not in value:
                raise KeyError("Adroit observation dict lacks state_observation.")
...
def _load_adroit_binary_dataset(config: TrainConfig) -> NumpyBatch:
    if config.adroit_binary_data_dir is None:
        raise ValueError(
            "adroit_binary_data_dir must point to the downloaded official "
            "demonstrations/offpolicy_hand_data directory."
        )
    root = Path(config.adroit_binary_data_dir)
    paths = [
        root / name
        for name in _binary_file_candidates(
            config.env_name, config.adroit_binary_include_bc
        )
    ]
    missing = [str(path) for path in paths if not path.exists()]
    if missing:
        raise FileNotFoundError("Missing Adroit-Binary dataset files: " + ", ".join(missing))
        ...
        for trajectory in trajectories:
            ...
            observations = _state_observation_array(
                _extract_trajectory_field(
                    trajectory, ("observations", "observations_n")
                )
            )
            actions = np.asarray(
                _extract_trajectory_field(trajectory, ("actions",)),
                dtype=np.float32,
            )
            rewards = np.asarray(
                _extract_trajectory_field(trajectory, ("rewards",)),
                dtype=np.float32,
            ).squeeze()

            # The official loader uses pos_ind=-1: retain the trajectory through
            # the last success reward, not the first, and mark every reward-0
            # transition as terminal for return-to-go recursion.
            ...
            try:
                next_observations = _state_observation_array(
                    _extract_trajectory_field(
                        trajectory,
                        ("next_observations", "next_observations_n"),
                    )
                )
                current_obs = observations[: last + 1]
                next_obs = next_observations[: last + 1]
            except KeyError:
                if len(observations) < last + 2:
                    raise ValueError(
                        "Adroit trajectory has no next_observations and does "
                        "not contain one extra observation."
                    )
                current_obs = observations[: last + 1]
                next_obs = observations[1 : last + 2]

            clipped_actions = actions[: last + 1]
            kept_rewards = rewards[: last + 1]
            terminals = np.isclose(kept_rewards, 0.0)
            episodes.append(
                _episode_to_arrays(
                    list(current_obs),
                    list(clipped_actions),
                    list(kept_rewards),
                    list(next_obs),
                    list(terminals),
                    config,
...
def collect_trajectory(
    ...
    for step in range(maximum_steps):
        ...
        success = _is_goal_achieved(info) if _is_adroit_binary(
            config.env_name, config.dataset_source
        ) else False
        terminal = bool((done and not timeout) or success)
...
def _build_offline_dataset(env: gym.Env, config: TrainConfig) -> ArrayDataset:
    if config.dataset_source == "adroit_binary":
        arrays = _load_adroit_binary_dataset(config)
    else:
        arrays = _load_d4rl_trajectory_dataset(env, config)
    return ArrayDataset(arrays, seed=config.dataset_seed)
...
def train(config: TrainConfig) -> None:
    ...
    offline_dataset = _build_offline_dataset(train_env, config)
    online_buffer = ReplayBuffer(
    ...
    run_name = (
        ...
        f"calql{int(config.enable_calql)}_cql{config.cql_min_q_weight:g}"
    )
    ...
    print(
        ...
        f"device={device}\n"
        f"offline_dataset_size={offline_dataset.size}\n"
        f"actor_hidden_dims={config.actor_hidden_dims}\n"
        f"critic_hidden_dims={config.critic_hidden_dims}\n"
        f"CalQL={config.enable_calql}, CQL weight={config.cql_min_q_weight}, "
        f"Lagrange={config.cql_lagrange}\n"
        f"pretrain_steps={config.pretrain_steps}, "
        f"online_steps={config.num_total_steps}\n"
        f"mixing_ratio={config.mixing_ratio}, online_UTD={config.online_utd_ratio}\n"
        f"log_dir={log_dir}",
        flush=True,
    )
    ...
    for gradient_step in trange(
        config.pretrain_steps, desc="Cal-QL offline pre-training"
    ):
        ...
        if gradient_step % config.offline_eval_every == 0:
            eval_info = evaluate(
                actor,
                eval_env,
                device,
            ...
            message = (
                f"[offline eval] step={gradient_step}, "
    ...
    # Online fine-tuning. Complete trajectories are inserted before updates so
    # every online transition has a valid MC return-to-go lower bound.
    environment_steps = 0
    ...
    progress = trange(
        config.num_total_steps,
        desc="Cal-QL online fine-tuning",
    )
    while environment_steps < config.num_total_steps:
        ...
        while (
            ...
            trajectory, raw_return, trajectory_length = collect_trajectory(
                actor, train_env, device, config
            )
            # The official sparse-task loop inserts complete trajectories so
            # every stored transition keeps its correct return-to-go. The final
            # trajectory may make the interaction count exceed the nominal
            # budget by fewer than one episode, which is preferable to cutting
            # a trajectory after its returns have been computed.
            ...
            visible_increment = min(
                trajectory_length,
                max(config.num_total_steps - previous_environment_steps, 0),
            )
            progress.update(visible_increment)
        ...
        for _ in range(update_count):
            ...
            # Released configurations keep CQL/Cal-QL active online, while
            # the official main script also exposes an SAC-only switch.
            last_metrics = learner.update(
                torch_batch, use_cql=online_use_cql
            )
        ...
        if environment_steps >= next_eval_step:
            eval_info = evaluate(
                actor,
                eval_env,
                device,
            ...
            normalized = _get_normalized_score(eval_env, eval_info["return"])
            message = (
                f"[online eval] env_step={environment_steps}, "
                f"gradient_step={online_gradient_steps}, "
                f"return={eval_info['return']:.3f}, "
                f"length={eval_info['length']:.1f}"
            )
            if normalized is not None:
            ...
            print(message, flush=True)
            writer.flush()
            next_eval_step = environment_steps + config.online_eval_every
