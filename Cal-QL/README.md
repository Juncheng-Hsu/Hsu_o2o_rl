...
online budget 等实验协议参数不会因为环境不同而自动修改；offline
pre-training budget 则按论文/官方运行设置由环境类别自动确定。
...
默认 offline pre-training budget：

```text
AntMaze              = 1,000,000
MuJoCo locomotion    = 1,000,000
Adroit family        =    20,000
```

这里的标准 D4RL Adroit human/cloned/expert 是对论文 Adroit-Binary 任务的
benchmark adaptation，并继承论文 Adroit 类别的 20K offline budget。
