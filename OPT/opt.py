...
def _apply_environment_config(config: TrainConfig) -> None:
    ...
    if "antmaze" in env:
        ...
        elif env == "antmaze-medium-diverse-v2":
...
def train(config: TrainConfig) -> None:
    ...
    for online_step in trange(config.online_steps, desc="OPT online phase"):
        ...
        if online_buffer.size >= config.n_tau:
            ...
            # N_tau is a genuine interaction warm-up: all N_tau collection
            # steps are part of ``online_steps`` but perform no regular online
            # fine-tuning.  Updates begin on the following interaction.
            if online_buffer.size > config.n_tau:
