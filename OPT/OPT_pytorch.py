...
This file ports the paper and the author-released implementation of:

    Online Pre-Training for Offline-to-Online Reinforcement Learning
    (ICML 2025).

The implementation keeps the user's current single-file PyTorch style while
preserving the official algorithmic structure:

1. Offline pre-training learns a deterministic policy and an ``offline`` twin
   critic using TD3+BC (MuJoCo) or SPOT (AntMaze/Adroit).
2. The policy is frozen for ``n_tau`` online interactions. These interactions
   form the small online dataset used by online pre-training.
3. A separately initialized ``online-pretrained`` twin critic is meta-trained
   with an offline TD loss followed by a differentiable inner update and an
   online TD loss, matching Equation (3) and the released ``Hot_Plug`` code.
4. Online fine-tuning updates both critic pairs. The actor maximizes a weighted
   combination ``(1-kappa) Q_off + kappa Q_on`` as in Equation (4).
5. Balanced replay is reproduced with the official density-ratio objective and
   density-ratio priorities over a replay buffer containing the offline dataset
   and all collected online transitions.
6. The TD3 path and the SPOT path are both implemented. The default dataclass
   values reproduce the released ``hopper-medium-v2`` TD3+OPT configuration.
   A named ``antmaze-umaze-v2`` preset reproduces the released SPOT+OPT config.

The official repository is already written in PyTorch. This file is therefore
not a framework translation; it is a cleaned, self-contained reorganization of
that implementation into the user's TrainConfig/TensorBoard/single-file style.
PyTorch versions, Gym/D4RL versions, and random-number streams can still prevent
...
@dataclass
class TrainConfig:
    # The defaults reproduce configs/opt/hopper/medium_v2.yaml.
    env_name: str = "hopper-medium-v2"
    official_preset: Optional[str] = None
    ...
    # The released opt.py constructs the balanced priority buffer with its
    # separate default capacity of one million transitions.
    ...
    # TD3+BC offline objective.
    td3_bc_alpha: float = 2.5

    # SPOT VAE and support constraint.
    ...
    log_root: str = "logs/OPT_PyTorch"
    device: str = "cuda:0" if torch.cuda.is_available() else "cpu"


def _apply_official_preset(config: TrainConfig) -> None:
    """Apply a released YAML configuration without hiding task assumptions.

    The default dataclass already matches hopper-medium-v2. Named presets are
    provided for two representative released configs whose values are useful
    when switching domains.
    """

    if config.official_preset is None:
        return

    name = config.official_preset.lower()
    if name == "hopper-medium-v2":
        ...
        config.kappa = 0.1
        config.kappa_cool_steps = 100_000
        config.kappa_end = 0.9
        ...
        raise ValueError(
            "Unknown official_preset. Supported presets are "
            "hopper-medium-v2, walker2d-random-v2, and antmaze-umaze-v2."
        )
