...
def _apply_env_config(config: TrainConfig) -> None:
    ...
    if domain == "antmaze":
        ...
        config.online_policy_noise = {
            "hopper-random-v2": 0.1,
        ...
        config.critic_hidden_layers = (
            3
            if name in {"hammer-cloned-v1", "relocate-cloned-v1"}
            else 2
        )
        config.policy_noise = 0.0
        config.online_policy_noise = 0.0
...
class PARSLearner:
    ...
    def _target_action(
        ...
        target_policy = self.actor if online else self.actor_target
        return (
                target_policy(next_state) + noise
